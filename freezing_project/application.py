...
"""
    input_features = [latitude, longitude, year, month, day, hour, wind_speed, temperature, humidity]

    # Predict freezing probability
    prediction = model.predict([input_features])
    freezing_status = int(prediction[0])

    # Map prediction to human-readable status
    freezing_map = {
        0: "No freezing",
        1: "Freezing possible",
        2: "Freezing confirmed"
    }

    return jsonify({
        "fcst_time": date,#f"{year:04}-{month:02}-{day:02} {hour:02}:00",
        "temperature": temperature,
        "humidity": humidity,
        "wind_speed": wind_speed,
        "freezing_status": freezing_map[freezing_status]
    })
